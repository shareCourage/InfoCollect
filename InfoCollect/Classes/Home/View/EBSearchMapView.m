...
//
#define kImageName_LocationBtn @"home_current_hL"
...
- (void)mapViewDidAppear {
    ...
    if (![PHTool locationEnable]) {//开启了定位，这里就不用设置地图中心了
        CLLocationCoordinate2D centerCL = [PHUseInfo sharedPHUseInfo].userLocation;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        [PHTool locationEnable] ? [self addLocationButton] : nil;
...
- (void)addLocationButton {
    ...
    [location setImage:[UIImage imageNamed:kImageName_LocationBtn] forState:UIControlStateNormal];
...
- (void)addCenterImageViewMethod {
    ...
    UIImage *centerImage = [UIImage imageNamed:@"icon_line_begin@2x"];
...
- (void)didMoveToSuperview
{
    ...
    [PHTool locationEnable] ? [self setLocationBtnAutoLayout] : nil;
...
- (void)mapView:(MAMapView *)mapView regionDidChangeAnimated:(BOOL)animated {
    PHLog(@"centerCoordinate ->  %.6f, %.6f, %.f", mapView.centerCoordinate.latitude, mapView.centerCoordinate.longitude, mapView.zoomLevel);
